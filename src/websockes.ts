...
export const registerWebsocket: FastifyPluginCallback<{ getWSServer: (sessionId: string) => WebSocket, getSession: (sessionId: string) => ({ id: string }) }> = (app, opts, done) => {
  const ws = new WebSocket.Server({ server: app.server });
  ws.on('connection', (source, req) => {
    const [, sessionId] = req.url?.split('/ws/') || [];
    const target = opts.getWSServer(sessionId);
